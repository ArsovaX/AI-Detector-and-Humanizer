// ai-detector.js — AI text detection engine with 7 sub-analyzers
...
// Sub-analyzer 1: Perplexity Analysis (weight 0.20)
function analyzePerplexity(tokens) {
  ...
  // Low perplexity + low variation = AI-like
  let score = 0;
  score += smoothStep(50, 10, avgPerplexity) * 0.6; // Low perplexity → high score
  score += smoothStep(0.5, 0.1, perplexityCV) * 0.4; // Low variation → high score
...
// Sub-analyzer 2: Burstiness Analysis (weight 0.20)
function analyzeBurstiness(tokens) {
  ...
  // Transition rate: how often adjacent sentences differ significantly
  let transitions = 0;
  ...
  // Low CV + low transition rate = AI-like
  let score = 0;
...
// Sub-analyzer 3: Vocabulary Richness (weight 0.15)
function analyzeVocabulary(tokens) {
  ...
  // Type-Token Ratio (normalized for text length using root TTR)
  ...
  // AI tends toward moderate TTR, lower hapax ratio, higher Yule's K
  let score = 0;

  // TTR: AI tends to have root-TTR between 4-7, humans often higher
  score += smoothStep(9, 4, ttr) * 0.3;

  // Hapax: AI uses fewer unique words, hapax < 0.5 is AI-like
  score += smoothStep(0.65, 0.35, hapaxRatio) * 0.35;

  // Yule's K: higher = less diverse. AI typically > 100
...
// Sub-analyzer 4: Sentence Structure (weight 0.15)
...
// Sub-analyzer 5: Repetition Detection (weight 0.10)
...
// Sub-analyzer 6: AI Phrase Detection (weight 0.10)
...
// Sub-analyzer 7: Formality/Tone Consistency (weight 0.10)
function analyzeTone(tokens) {
  ...
  // Low CV = monotone formal register (AI-like)
  let score = smoothStep(0.35, 0.08, formalityCV) * 0.6;
  // High average formality is also AI-like
...
// Per-sentence AI scoring for heatmap
function scoreSentences(tokens, results) {
  const { sentences, lowerWords } = tokens;
  ...
  for (const fp of (results.aiPhrases.flaggedPhrases || [])) {
    flaggedPositions.add(fp.position);
  }
  ...
  return sentences.map((s, i) => {
    ...
    if (stdLength > 0 && burstLengths[i] !== undefined) {
      ...
      score += smoothStep(2, 0, lenZ) * 0.2; // Close to mean → AI-like
      factors += 0.2;
    }
...
export function analyzeText(text) {
  ...
  // Pull overall score slightly toward 0.5 (conservative) so only
  // strong signals push it to extremes
  overallScore = 0.5 + (overallScore - 0.5) * 0.85;

  // --- Confidence: multi-factor ---
  const scoreStd = standardDeviation(scores);

  // 1. Analyzer agreement (low std = high agreement)
  const agreementConf = clamp(1 - scoreStd * 2.0);

  // 2. Evidence strength: how far the overall score is from 0.5
  //    (scores near 0.5 are inherently less confident)
  const extremity = Math.abs(overallScore - 0.5) * 2; // 0..1
  const evidenceConf = smoothStep(0, 0.6, extremity);

  // 3. Length factor: ramp up from 0 at 30 words to 1 at 250 words
  const wordCount = tokens.words.length;
  const lengthConf = clamp((wordCount - 30) / 220);

  // 4. Sentence count factor: need at least a few sentences
